...
def input_vars():    
    ...
    while option != 1 or option != 2:
        if option == 1:
            ...
            for i in range(3):
                if i == 0:
                    m = int(next(f).split()[0])    # Reads the dimensions
                if i == 1:
                    otype = next(f).split()
                    otype = otype[0]
                    if otype != "min" and otype != "max":
            ...
            tmp_array = []
            # Read the line with volumes
            tmp_array = (f.readline()).split()
            ...
            for j in range(m + 2):
                if j < m:
                    in_A.append(int(tmp_array[j]))
                elif j == m:
                    sign = tmp_array[j]
                elif j == (m + 1):
                    in_b = int(tmp_array[j])
            
            # Reading solution set
            tmp = (f.readline()).split()[0]
            if tmp != '0/1' and tmp.lower() != 'z':
                print("Given feasable solution set is not valid! Should be either '0/1' or 'Z'")
                print("Default solution set. Set: 0/1")
                sol_set = '0/1'
            else:
                sol_set = tmp
            # Reading direction
            tmp = (f.readline()).split()[0]
            if tmp != 'forward' and tmp != 'backward':
                print("Given direction is not valid! Should be either 'forward' or 'backward'")
                print("Default direction set. Direction: forward")
                direction = 'forward'
            else:
                direction = tmp
    ...
    return m, otype, in_c, in_A, sign, in_b, sol_set, direction
